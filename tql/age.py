...
def run_stardate(iso_params: dict, prot: tuple=None, nsteps: int=1000,
                 return_age: bool=True, verbose: bool=True):
    ...
    star = Star(iso_params, prot=prot, prot_err=prot_err)  # Here's where you add a rotation period
...
def parse_samples(star, return_age: bool=True, return_mass: bool=False,
            return_feh: bool=True, return_lndist: bool=True,
            return_Av: bool=True, verbose: bool=True):
    ...
    if return_mass:
        mass, mass_errp, mass_errm, mass_samples = star.mass_results(burnin=10)  # burnin is thin_by x larger than this
        if verbose:
    ...
    if return_lndist:
        lndistance, lndistance_errp, lndistance_errm, lndistance_samples = star.distance_results(burnin=10)
        if verbose:
