...
class TOIParams:
    ...
    def __init__(self, target_coord=None, toi_id=None, tic_id=None, gaia_id=None,
            ...
            if len(self.toi_params)>0:
                if self.target_coord is None:
                    self.target_coord = SkyCoord(ra=self.toi_params['RA'],
                                    ...
                                    unit=(u.hourangle, u.degree))[0]
                #set values if TOI
                #photometry
                self.Tmag = self.toi_params['TESS Mag'].values[0]*u.mag
                #parallax & proper motion
                self.pmra = self.toi_params['PM RA (mas/yr)'].values[0]*u.mas/u.year
                self.pmdec = self.toi_params['PM Dec (mas/yr)'].values[0]*u.mas/u.year
                self.distance = self.toi_params['Stellar Distance (pc)'].values[0]*u.pc
                self.parallax = Distance(self.distance).parallax.value*u.mas
                #stellar parameters
                self.Teff_tic = self.toi_params['Stellar Eff Temp (K)'].values[0]*u.K
                self.Rstar_tic = self.toi_params['Stellar Radius (R_Sun)'].values[0]*u.Rsun
        ...
        #         raise ValueError(f'{self.target_coord} does not match TOI Release catalog!')
            ...
            #position match
            all_tois = tql.get_tois(clobber=False, verbose=False,
                                    remove_known_planets=False,
                                    remove_FP=True)
            toi_coords = SkyCoord(ra=all_tois['RA'], dec=all_tois['Dec'],
            ...
            if len(self.toi_params)==0:
                if self.verbose:
                    print(f'{self.target_coord} does not match TOI Release catalog!')
...
class TICParams(TOIParams):
    ...
    def query_tic(self, radius=None):
        ...
        cat = Catalogs.query_region(self.target_coord, radius=radius,
    ...
    def match_single_star(self):
        if len(self.tic_catalog)==0:
            raise ValueError(f'{self.target_coord} has NO match in TIC catalog!')
            ...
            if self.gaia_id is not None:
                ...
                tic_coords = SkyCoord(ra=self.tic_catalog['ra'],
                                      dec=self.tic_catalog['dec'],
                                      unit=(u.degree))
                separation = target_coord.separation(tic_coords)
                idx = separation < self.search_radius/2.
                self.tic_catalog = self.tic_catalog.loc(idx)
                if len(self.tic_catalog)==0:
                    if self.verbose:
                        print(f'{self.target_coord} does not match any star in TIC catalog!')
                elif len(self.tic_catalog)>1:
                    #take the nearest tic
                    self.tic_catalog = self.tic_catalog.iloc[separation.argmin()]
    ...
    def match_single_star(self):
        if self.gaia_id is not None:
            ...
            if len(star)==0:
                ...
                star_coords = SkyCoord(ra=star['ra'], dec=star['dec'], unit='deg')
